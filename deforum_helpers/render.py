...
from ainodes_frontend.base import handle_ainodes_exception
...
from .qops import pixmap_to_pil_image, pil_image_to_pixmap
...
def render_animation(node, args, anim_args, video_args, parseq_args, loop_args, controlnet_args, animation_prompts, root, callback=None):
    ...
    if anim_args.animation_mode in ['2D','3D']:
        ...
        if anim_args.video_init_path == None and (anim_args.hybrid_composite != 'None' or anim_args.hybrid_motion in ['Affine', 'Perspective', 'Optical Flow']):
            anim_args.hybrid_composite = 'None'
            anim_args.hybrid_motion = 'None'
            print("DEFORUM WARNING")
            print("     Hybrid Video requested, but no init video was provided, turning Hybrid Video off.")
    ...
    while frame_idx < anim_args.max_frames:
        ...
        if turbo_steps > 1:
            ...
            for tween_frame_idx in range(tween_frame_start_idx, frame_idx):
                ...
                current_image = Image.fromarray(cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_BGR2RGB))
        ...
        hybrid_available = (anim_args.hybrid_composite != 'None' or anim_args.hybrid_motion in ['Optical Flow', 'Affine', 'Perspective']) and anim_args.video_init_path is not None
        ...
        if prev_img is not None:
            ...
            if anim_args.hybrid_composite == 'Normal' and anim_args.video_init_path is not None:
                args, prev_img = hybrid_composite(args, anim_args, frame_idx, prev_img, depth_model, hybrid_comp_schedules, root)
...
def render_animation_refactor(node, args, anim_args, video_args, parseq_args, loop_args, controlnet_args, animation_prompts, root, callback=None):
    ...
    # load depth model for 3D
    anim_args.predict_depths = (anim_args.animation_mode == '3D' and anim_args.use_depth_warping) or anim_args.save_depth_maps
    anim_args.predict_depths = anim_args.predict_depths or (anim_args.hybrid_composite and anim_args.hybrid_comp_mask_type in ['Depth','Video Depth'])
    if anim_args.predict_depths:
        keep_in_vram = True
        device = ('cpu' if lowvram or medvram else root.device)
        depth_model = DepthModel(root.models_path, device, root.half_precision, keep_in_vram=keep_in_vram, depth_algorithm=anim_args.depth_algorithm, Width=args.W, Height=args.H, midas_weight=anim_args.midas_weight)
        # depth-based hybrid composite mask requires saved depth maps
    ...
    while frame_idx < anim_args.max_frames:
        frame = anim_frame(frame_idx, args, anim_args, root, keys, depth_model=depth_model)

    if anim_args.predict_depths and not keep_in_vram:
        depth_model.delete_model() # handles adabins too
...
def anim_frame(frame_idx, args, anim_args, root, keys, depth_model=None):
    print(f"\033[36mAnimation frame: \033[0m{frame_idx}/{anim_args.max_frames}  ")
    ...
    if anim_args.animation_mode == '3D':
        torch_gc()
        if anim_args.predict_depths: depth_model.to(root.device)
    ...
    hybrid_available = (anim_args.hybrid_composite != 'None' or anim_args.hybrid_motion in ['Optical Flow', 'Affine',
                                                                                            'Perspective']) and anim_args.video_init_path is not None
    if anim_args.color_coherence == 'Video Input' and hybrid_available:
        if int(frame_idx) % int(anim_args.color_coherence_video_every_N_frames) == 0:
            prev_vid_img = Image.open(os.path.join(args.outdir, 'inputframes',
    ...
    if prev_img is not None:
        ...
        # do hybrid compositing before motion
        if anim_args.hybrid_composite == 'Before Motion':
            args, prev_img = hybrid_composite(args, anim_args, frame_idx, prev_img, depth_model, hybrid_comp_schedules,
                                              root)
        ...
        if anim_args.hybrid_motion in ['Optical Flow']:
            if anim_args.hybrid_motion_use_prev_img:
                flow = get_flow_for_hybrid_motion_prev(frame_idx - 1, (args.W, args.H), inputfiles, hybrid_frame_path,
                                                       prev_flow, prev_img, anim_args.hybrid_flow_method, raft_model,
                                                       anim_args.hybrid_flow_consistency,
                                                       anim_args.hybrid_consistency_blur,
                                                       anim_args.hybrid_comp_save_extra_frames)
            else:
                flow = get_flow_for_hybrid_motion(frame_idx - 1, (args.W, args.H), inputfiles, hybrid_frame_path,
                                                  ...
                                                  anim_args.hybrid_flow_consistency, anim_args.hybrid_consistency_blur,
                                                  anim_args.hybrid_comp_save_extra_frames)
            prev_img = image_transform_optical_flow(prev_img, flow, hybrid_comp_schedules['flow_factor'])
            prev_flow = flow
        ...
        # apply frame noising
        if args.use_mask or anim_args.use_noise_mask:
            args.noise_mask = compose_mask_with_check(root, args, noise_mask_seq, noise_mask_vals,
                                                      Image.fromarray(cv2.cvtColor(contrast_image, cv2.COLOR_BGR2RGB)))
        noised_image = add_noise(contrast_image, noise, args.seed, anim_args.noise_type,
    ...
    if anim_args.use_mask_video:
        args.mask_file = get_mask_from_file(get_next_frame(args.outdir, anim_args.video_mask_path, frame_idx, True),
                                            args)
        args.noise_mask = get_mask_from_file(get_next_frame(args.outdir, anim_args.video_mask_path, frame_idx, True),
                                             args)
    ...
    if args.use_mask:
        args.mask_image = compose_mask_with_check(root, args, mask_seq, mask_vals,
                                                  args.init_sample) if args.init_sample is not None else None  # we need it only after the first frame anyway
    # setting up some arguments for the looper
    loop_args.imageStrength = loopSchedulesAndData.image_strength_schedule_series[frame_idx]
    loop_args.blendFactorMax = loopSchedulesAndData.blendFactorMax_series[frame_idx]
    loop_args.blendFactorSlope = loopSchedulesAndData.blendFactorSlope_series[frame_idx]
    ...
    if anim_args.optical_flow_redo_generation != 'None' and prev_img is not None and strength > 0:
        print(
            f"Optical flow redo is diffusing and warping using {anim_args.optical_flow_redo_generation} optical flow before generation.")
        stored_seed = args.seed
        args.seed = random.randint(0, 2 ** 32 - 1)

        disposable_image = generate(node, args, keys, anim_args, loop_args, controlnet_args, root, frame_idx,
    ...
    if int(anim_args.diffusion_redo) > 0 and prev_img is not None and strength > 0:
        ...
        gc.collect()
    # generation
    image = generate(node, args, keys, anim_args, loop_args, controlnet_args, root, frame_idx,
                     sampler_name=scheduled_sampler_name)
    if image is None:
        return None
    # do hybrid video after generation
    ...
    # color matching on first frame is after generation, color match was collected earlier, so we do an extra generation to avoid the corruption introduced by the color match of first output
    if frame_idx == 0 and (
            anim_args.color_coherence == 'Image' or (anim_args.color_coherence == 'Video Input' and hybrid_available)):
...
def generate(node, args, keys, anim_args, loop_args, controlnet_args, root, frame=0, return_sample=False, sampler_name=None):
    image = generate_inner(node, args, keys, anim_args, loop_args, controlnet_args, root, frame, return_sample, sampler_name)
    return image
...
def deforum_ainodes_adapter(node, data=None):
    ...
    try:
        test = render_animation(node, args, anim_args, video_args, parseq_args, loop_args, controlnet_args,
                                animation_prompts, root, callback=node.handle_callback)
        return True
    except Exception as e:
        print(repr(e))
